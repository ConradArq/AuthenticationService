...
{
    public class EmailConfirmationUserEmailService : IUserEmailService<EmailConfirmationEmailDto>
    {
        private readonly IServiceScopeFactory _serviceScopeFactory;
        private readonly IEmailQueueService _emailQueueService;

        public EmailConfirmationUserEmailService(IServiceScopeFactory serviceScopeFactory, IEmailQueueService emailQueueService)
        {
        ...
        public void SendEmail(object applicationUserId)
        {
            ...
            Func<Task<Email>> buildEmailFuncAsync = async () =>
            {
                using (var scope = _serviceScopeFactory.CreateScope())
                {
                    ...
                    string? emailTemplateFileName = configuration["Paths:Templates:Emails:Files:EmailConfirmation"];
                    ...
                    var emailDto = mapper.Map<EmailConfirmationEmailDto>(applicationUser);
                    var emailConfirmationToken = await userManager.GenerateEmailConfirmationTokenAsync(applicationUser);
                    emailDto.Token = WebUtility.UrlEncode(emailConfirmationToken);
                    ...
                    email.Subject = EmailTemplateMessages.EmailConfirmationEmailSubject;
        ...
        public void SendNotification(object applicationUserId)
        {
            ...
            //multipartContent.Add(new StringContent("1"), "TemplateId");
            //multipartContent.Add(new StringContent(applicationUser.Email), "To");

            //var jsonPayload = JsonSerializer.Serialize(emailConfirmationEmailDto);
            //multipartContent.Add(new StringContent(jsonPayload, Encoding.UTF8, "application/json"), "TemplatePlaceholderMappings");

            //await _httpService.SendAsync<object>(
            //    HttpMethod.Post,
            //    "https://localhost:7028/api/emailnotification/send",
            //    multipartContent,
            //    "multipart/form-data",
            //    new Dictionary<string, string>
            //    {
            //        { "Authorization", $"Bearer {_jwtTokenProvider.GetUserAuthenticationToken()}" }
            //    }
            //);
        }
    }
