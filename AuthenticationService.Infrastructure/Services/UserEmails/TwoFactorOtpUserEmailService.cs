...
{
    public class TwoFactorOtpUserEmailService : IUserEmailService<TwoFactorOtpEmailDto>
    {
        private readonly IServiceScopeFactory _serviceScopeFactory;
        private readonly IEmailQueueService _emailQueueService;
        private readonly IOtpService _otpService;

        public TwoFactorOtpUserEmailService(IServiceScopeFactory serviceScopeFactory, IEmailQueueService emailQueueService, IOtpService otpService)
        {
        ...
        public void SendEmail(object applicationUserId)
        {
            ...
            Func<Task<Email>> buildEmailFuncAsync = async () =>
            {
                using (var scope = _serviceScopeFactory.CreateScope())
                {
                    ...
                    string? emailTemplateFileName = configuration["Paths:Templates:Emails:Files:TwoFactorOtp"];
                    ...
                    var emailDto = mapper.Map<TwoFactorOtpEmailDto>(applicationUser);
                    var (otp, expirationMinutes) = await _otpService.GenerateAndStoreOtpAsync(applicationUser.Id);
                    emailDto.Otp = otp;
                    emailDto.ExpirationMinutes = expirationMinutes.ToString();
                    ...
                    email.Subject = EmailTemplateMessages.TwoFactorOtpEmailSubject;
